...
export default function CategoryList({ data }) {
  ...
  if (data && data.length != 0) {
    return (
      <>
      <div id="category-list" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-10">
        {list.map((item, index) => (
      ...
      {data.length>limitLength ? <button className="btn max-md:w-full md:px-20 mt-12 py-3 px-5 bg-gray-200" onClick={expand}><p class="text-base">Xem thêm</p></button>:''}
      </div>
